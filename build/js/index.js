...
var Index =
{
  init:function()
  {
    ...
    $('.accordionItem', '.section6').not(':first').find('.text').hide();

    $('input[name=phone]').mask('+999999999999?9999');

    setTimeout(function(){
      $('.progressBar').addClass('run');
      $('#progressInteger').spincrement({
        from: 0,
        duration: 13500
      })
    }, 2000);
  ...
  countdown: function(){
    ...
    if($.cookie('timer')){
      ...
      var dateEnd = returnEndDate(0,40,50);
      $.cookie('timer', dateEnd);
    }

    dateEnd = Date.parse(dateEnd) / 1000;
    myDate = Date.parse(myDate) / 1000;
    ...
    $('#countdown').countdown({until: lastDate, format: 'H,M,S'});
  },
